...
**Source of truth:** [`configs/corruptions/suite.yaml`](../configs/corruptions/suite.yaml).
`cdlib.metrics.corruptions` reads that file. Do not edit numbers only here.
...
RNG seed is `(pair_id, corruption, severity, direction)` so every configuration sees the same corrupted pair.
...
| h264 CRF (optional) | 18 | 23 | 28 | 35 | 45 |
| hevc CRF (optional) | 20 | 25 | 30 | 37 | 47 |
| shadow intensity | 0.1 | 0.2 | 0.35 | 0.5 | 0.7 |
| viewpoint translate / rotate | 1% / 0.5° | 2% / 1° | 4% / 2° | 8% / 4° | 16% / 8° |
| occlusion area | 2% | 5% | 10% | 20% | 35% |

Paper grid (main): brightness, gamma, blur (gaussian + motion), jpeg, viewpoint, occlusion. Codecs stay under `optional` in the YAML.
...
Visual calibration: dump one example per (name, severity) from a held-out pair and check that s1 is subtle and s5 is ugly-but-recognisable. Tick this off before quoting retention numbers.
